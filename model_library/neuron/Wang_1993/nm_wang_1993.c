...
void wang_ina_f (double * vars, double * params, double * ret) {
    *ret = params[WANG_G_NA] * pow(wang_mna(vars[WANG_V], params[WANG_SIGMA]), 3) * vars[WANG_H_NA] * (vars[WANG_V] - params[WANG_V_NA]);
    return;
}
...
void wang_inap_f (double * vars, double * params, double * ret) {
    *ret = params[WANG_G_NAP] * wang_inf_f(vars[WANG_V], 51.0, 5.0) * (vars[WANG_V] - params[WANG_V_NA]);
    return;
}
...
void wang_f (double * vars, double * ret, double * params, double syn) {
    ...
    ret[WANG_H_NA] = wang_hna(vars[WANG_V], params[WANG_SIGMA], params[WANG_PHI], vars[WANG_H_NA]);
    ret[WANG_N_K] = wang_nk(vars[WANG_V], params[WANG_SIGMA], params[WANG_PHI], vars[WANG_N_K]);
    ret[WANG_M_KS] = wang_mks(vars[WANG_V], 1.0, params[WANG_TAUM], vars[WANG_M_KS]);
    ret[WANG_H1_KS] = wang_h1ks(vars[WANG_V], 1.0, vars[WANG_H1_KS]);
    ret[WANG_H2_KS] =  wang_h2ks(vars[WANG_V], 1.0, vars[WANG_H2_KS]);
...
void wang_1993 (neuron_model nm, double syn) {
    nm.method(wang_f, nm.dim, nm.params[WANG_DT], nm.vars, nm.params, syn);
    return;
}
...
void wang_init (neuron_model * nm, double * vars, double * params) {
    ...
    nm->vars[WANG_H_NA] = wang_hna_inf (nm->vars[WANG_V], nm->params[WANG_SIGMA]); /* Steady-state */
    nm->vars[WANG_N_K] = wang_nk_inf (nm->vars[WANG_V], nm->params[WANG_SIGMA]); /* Steady-state */
    nm->vars[WANG_M_KS] = wang_inf_f(nm->vars[WANG_V], 34.0, 6.5); /* Steady-state */
    nm->vars[WANG_H1_KS] = wang_inf_f(-nm->vars[WANG_V], -65.0, 6.6); /* Steady-state */
    nm->vars[WANG_H2_KS] =  wang_inf_f(-nm->vars[WANG_V], -65.0, 6.6); /* Steady-state */
