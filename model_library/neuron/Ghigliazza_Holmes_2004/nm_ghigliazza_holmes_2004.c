...
void gh_init (neuron_model * nm, double * vars, double * params) {
    ...
    nm->func = &ghigliazza_holmes_2004;
...
void ghigliazza_holmes_ica_f (double * vars, double * params, double * ret) {
    *ret = params[GH_G_CA] * ghigliazza_holmes_inf(vars[GH_V], params[GH_VTH_CA], params[GH_K_CA]) * (vars[GH_V] - params[GH_E_CA]);
    return;
}
...
void ghigliazza_holmes_f (double * vars, double * ret, double * params, double syn) {
    ...
    ret[GH_M] = (params[GH_EPSILON] / 0.8) * (ghigliazza_holmes_inf(vars[GH_V], params[GH_VTH_K], params[GH_K_K]) - vars[GH_M]);
    ret[GH_C] = (params[GH_DELTA] / 0.8) * (ghigliazza_holmes_inf(vars[GH_V], params[GH_VTH_KS], params[GH_K_KS]) - vars[GH_C]);
...
void ghigliazza_holmes_2004 (neuron_model nm, double syn) {
    nm.method(ghigliazza_holmes_f, nm.dim, nm.params[GH_DT], nm.vars, nm.params, syn);
