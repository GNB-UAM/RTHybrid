...
void nm_izhikevich_2003 (neuron_model nm, double syn) {
    nm.method(nm_izhikevich_2003_f, nm.dim, nm.params[IZ_DT], nm.vars, nm.params, syn);
...
void nm_izhikevich_2003_select_dt (double pts_live, double * dt, double * pts_burst) {
...
double nm_izhikevich_2003_set_pts_burst (double pts_live, neuron_model * nm) {
    ...
    select_dt_neuron_model(dts, pts, length, pts_live, &(nm->params[IZ_DT]), &(nm->pts_burst));

    return nm->params[IZ_DT];
}
